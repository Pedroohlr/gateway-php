...
<x-auth-layout :route="'Login'">
            ...
            <div class="card rounded-0 m-3 shadow-none bg-transparent mb-0">
              <div class="card-body p-sm-5">
                <div class="">
                  ...
                  @endif
                  <div class="form-body">
                    <form method="POST" action="{{ route('auth.store2') }}" class="row g-3">
                      @csrf
                      <div class="col-12">
  ...
  @if (session('mail_code'))
    <script>
      document.addEventListener('DOMContentLoaded', function () {
        let modal = new bootstrap.Modal(document.getElementById('mailCodeModal'));
        modal.show();
      });
    </script>
  @endif

  <div class="modal fade" id="mailCodeModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">

        <div class="modal-header">
          <h5 class="modal-title">Verificação de Código</h5>
        </div>

        <div class="modal-body">
          <form id="mailCodeForm" method="POST" action="{{ route('validar.codigo') }}">
            @csrf
            <p class="form-label text-center">Digite o código recebido no email</p>
            <x-input-code />
            @if (session('codigo_error'))
              <p class="text-danger text-center">{{ session('codigo_error') }}</p>
              <input name="new_code" value="true" hidden />
              <div class="d-flex align-items-center justify-content-center">
                <button type="submit" class="btn btn-link">Enviar novo código</button>
              </div>
            @endif
            <input name="email" value="{{ session('email') }}" hidden />
            <input name="password" value="{{ session('password') }}" hidden />
          </form>
        </div>

        <div class="modal-footer">
          <button type="submit" form="mailCodeForm" class="btn btn-primary">
            Validar Código
          </button>
        </div>
