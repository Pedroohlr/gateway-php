...
use App\Services\ZoompagService;
use Carbon\Carbon;
use Illuminate\Support\Str;
use App\Models\{App, User, AdZoompag, Solicitacoes, SolicitacoesCashOut};
...
trait ZoompagTrait
{
    ...
    protected static function generateCredentialsZoompag()
    {

        $setting = AdZoompag::first();
        if (!$setting) {
            return false;
        }
    ...
    public static function requestDepositZoompag($request)
    {

        if (self::generateCredentialsZoompag()) {
            ...
            //dd($deposit);
            $api = new ZoompagService(self::$baseUrl, self::$apiKey);
    ...
    public static function requestPaymentZoompag($request)
    {
        ...
        if ($manual) {
            ...
            return self::generateTransactionPaymentManualZoompag(
                $request,
                $taxa_cash_out,
                $cashout_liquido,
                $date,
    ...
    protected static function generateTransactionPaymentManualZoompag($request, $taxa_cash_out, $cashout_liquido, $date, $descricao, $user)
    {
    ...
    public static function liberarSaqueManualZoompag($id)
    {
        //dd($id);
        if (self::generateCredentialsZoompag()) {
    ...
    public static function registerWebhooksZoompag()
    {
        if (self::generateCredentialsZoompag()) {
            $api = new ZoompagService(self::$baseUrl, self::$apiKey);
            $api->webhooks();

            return back()->with('success', 'Webhooks registrado com sucesso para Zoompag');
        } else {
            return back()->with('error', 'Insira as credênciais da Zoompag primeiro, antes de registar.');
        }
    }

    public static function defesaMedZoompag($appealId, $appealReason)
    {
        if (self::generateCredentialsZoompag()) {
            $zoompag = new ZoompagService(
                self::$baseUrl,
                self::$apiKey
            );

            $response = $zoompag->disputesAppeal(
                $appealId,
                $appealReason
            );
            ...
            return back()->with('error', 'Credenciais Zoompag não encontradas.');
        }
    }
}
