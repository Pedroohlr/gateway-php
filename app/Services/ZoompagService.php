...
class ZoompagService
{
    ...
    public function withdraw(WithdrawDTO $data)
    {
        ...
        Log::debug('[-][ZOOMPAGSERVICE][REQUESTPAYMENT][RESPONSE]: ' . json_encode($response->json()));
        return $response->json();
    }
    ...
    public function webhooks()
    {
        ...
        $payloadDeposit = [
            "url" => env('APP_URL') . "/zoompag/callback/deposit",
        ...
        \Log::debug("ZOOMPAG REGISTRO DE WEBHOOK DEPOSIT: " . json_encode($responseDeposit->json()));

        $payloadSaque = [
            "url" => env('APP_URL') . "/zoompag/callback/withdraw",
        ...
        \Log::debug("ZOOMPAG REGISTRO DE WEBHOOK WITHDRAW: " . json_encode($responseSaque->json()));
    ...
    public function disputesAll()
    {
        ...
        \Log::debug("ZOOMPAG REGISTRO DE DISPUTAS MED: " . json_encode($responseDisputes->json()));
    ...
    public function disputesOne($disputeId)
    {
        ...
        \Log::debug("ZOOMPAG REGISTRO DE DISPUTA MED PARA ID {$disputeId}: " . json_encode($responseDisputes->json()));
    ...
    public function disputesAppeal($disputeId, $appealReason)
    {
        ...
        \Log::debug("ZOOMPAG REGISTRO DE NOVO APELO PARA DISPUTA {$disputeId}: " . json_encode($responseDisputes->json()));
